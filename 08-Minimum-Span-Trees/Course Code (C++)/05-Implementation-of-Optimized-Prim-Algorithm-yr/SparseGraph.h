//
// Created by youren on 8/10/2020.
//
...
public:
    ...
    public:
        ...
        Edge<Weight>* begin(){
            index = 0;
            // ### 开始写成g[vertex].size() > 0是错误的，这里应该是调用邻接表才对
            if (g.adjList[vertex].size() > 0){    // 当vertex有邻边时才进行迭代
                return g.adjList[vertex][index];
            }
        ...
        Edge<Weight>* next(){
            ...
            index ++;  
            if (index < g.adjList[vertex].size()){
                return g.adjList[vertex][index];
            }
            return nullptr;
        }
        ...
        bool end(){
            // 应该是大于邻边的数量才结束
            return index >= g.adjList[vertex].size();
        }
    };
